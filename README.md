...
* `ahu_sul_catalog.json`: O recurso estruturado principal (formato JSON). Ele conecta o texto arquivístico bruto a metadados geográficos, dados temporais, códigos de referência arquivística modernizados e anotações sociolinguísticas algorítmicas.
  * **Chaves Principais (Core Keys):**
    ...
    * `vernacular_score`: Um número decimal (0.0 a 1.0) que representa quantitativamente o Score de Relevância Sociolinguística Potencial (SRSP), indicando a probabilidade de ocorrência de sintaxe vernácula/localizada no documento original.
    ...
    * `sociolinguistic_reasoning_by_deepseek_v3`: A justificativa analítica gerada pela IA para o Score de Relevância Sociolinguística Potencial (SRSP) atribuído.
...
* `auditoria/`: Pasta contendo a documentação metodológica e os dados da validação empírica (Auditoria de Consistência de IA) realizada sobre uma amostra aleatória de 10% do corpus (705 documentos). Inclui os dados utilizados para o cálculo do Erro Médio Absoluto, Correlação de Pearson e Viés Sistemático da IA.
...
* **Anotação:** Inferência Zero-shot por LLM (DeepSeek) para categorizar o remetente/destinatário, vetor de comunicação e calcular o Score de Relevância Sociolinguística Potencial (SRSP) junto com uma justificativa analítica por escrito.
* **Vetorização (Semântica):** Passagens enriquecidas com contexto transformadas em embeddings usando o modelo `intfloat/multilingual-e5-large` da HuggingFace.
